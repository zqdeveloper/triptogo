...
<div id="bo_list" style="width:<?php echo $width; ?>">
    ...
    <form name="fboardlist" id="fboardlist" action="<?php echo G5_BBS_URL; ?>/board_list_update.php"
        ...
        <div style="display: grid;grid-auto-flow: column;">
            <div class="map-wrapper" id="map" style="width:800px;background: red;border-radius:4px;">
                ...
                <script>
                    function initMap() {
                        const uluru = {lat: 12.97, lng: 77.59}
                        // The map, centered at Uluru
                        const map = new google.maps.Map(document.getElementById("map"), {zoom: 1, center: uluru,});
                        // The marker, positioned at Uluru
                        <?php

                        for ($i = 0; $i < count($list); $i++) {
                        $lat = $list[$i]['wr_1'];
                        $lan = (float)$list[$i]['wr_2'];
                        $place = $list[$i]['wr_3'];
                        $address = $list[$i]['placeName'];
                        $placeId = $list[$i]['placeId'];
                        $ca_name_href = $list[$i]['href'];
                        if ($lat != null && $lan != null && $ca_name_href!=null) {
                        ?>
                        var location = {lat: Number(<?php echo $lat;?>), lng: Number(<?php echo $lan; ?>)}
                        console.log(location);
                        var marker = addMarker(location, map, "" +<?php echo ($i + 1) . "";?>)
                        marker.addListener("click", () => {
                            map.setZoom(15);
                            map.setCenter(marker.getPosition());
                            native('<?php  echo  $ca_name_href;?>'.replaceAll('amp;',''));
                        })
                        <?php
                        }
                        }
            ...
            <div class="tbl_head01 tbl_wrap">
                <table>
                    ...
                    <tr>
                        ...
                        ">제목</th>
                        <th scope="col">글쓴이</th>
                        <th scope="col" style="display: none;"><?php echo subject_sort_link('wr_hit', $qstr2, 1) ?>
                            조회 </a></th>
                        <?php if ($is_good) { ?>
                            <th scope="col"><?php echo subject_sort_link('wr_good', $qstr2, 1) ?>추천 </a></th><?php } ?>
                        <?php if ($is_nogood) { ?>
                            <th scope="col"><?php echo subject_sort_link('wr_nogood', $qstr2, 1) ?>
                            비추천 </a></th><?php } ?>
                    ...
                    for ($i = 0; $i < count($list); $i++) {
                        ...
                        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?> <?php echo $lt_class ?>">
                            ...
                            <?php } ?>
                            <td class="td_num2" style="display: none;">
                                <?php
                            ...
                            <td class="td_num" style="display:none;"><?php echo $list[$i]['wr_hit'] ?></td>
                            <?php if ($is_good) { ?>
